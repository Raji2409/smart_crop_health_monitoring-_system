...
def dashboard():
    ...
    if request.method=="POST":
        ...
        img_arr = np.expand_dims(image.img_to_array(
            image.load_img(path,target_size=(224,224)))/255.0,0)
...
# Convert model output to dashboard label
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?)
        """,(session["username"],request.form["plant_name"],disease,
             stress_res,sev_level,sev_percent,soil,hum,temp,ph))
        conn.commit()
        conn.close()
    ...
    return render_template("user_dashboard.html",username=session["username"],**result)
...
def charts():
    ...
    # Healthy = 1, Stress = 0
    health = [1 if s == "Healthy" else 0 for s in df["stress"]]
